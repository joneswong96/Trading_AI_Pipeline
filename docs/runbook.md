...
- **Project A raw producers**：TradingView 仍只用 `POST /alert`。Exact
  `LIQ_V2/r9`、`EXP_V3/r5`、`EXP_SCANNER/r6`、`RENKO_V3_SNIPER/r1` JSON 會先入
  Product Input / offline Section 2 adapter，唔經 legacy parser、trigger 或 wake。
  Invalid recognized payload 回 bounded 4xx，唔會 fall through。Scanner 只係
  non-promoting quality evidence。`PROJECT_A_RAW_PRODUCER_INGEST_ENABLED`
