...
- **Project A raw producers**：TradingView 仍只用 `POST /alert`。三個主動生產者
  `LIQ_V2/r9`、`EXP_V3/r5`、`RENKO_V3_SNIPER/r1` JSON 會先入
  Product Input / offline Section 2 adapter，唔經 legacy parser、trigger 或 wake。
  Invalid recognized payload 回 bounded 4xx，唔會 fall through。`EXP_SCANNER/r6`
  只保留嚴格 dormant compatibility adapter，唔 materialize、唔開 alert、唔投票、
  唔會係 Section 2 建立/晉級 story 條件。`PROJECT_A_RAW_PRODUCER_INGEST_ENABLED`
  default `true` 只開 immutable telemetry/state ingest；capture、provider、writer、
  Telegram、Notion、MT5 同 order 仍然 disabled。部署前已運行嘅 non-reload
  process 唔會自動取得呢段 code。
