...
- **EXP_V3 / LIQ_V2 compatibility**：bounded Expansion text and `engine=LIQ_V2`
  JSON are classified as telemetry only. Expansion `UP/DOWN` is movement and
  never LONG/SHORT; Liquidity bare `price` is a level, never market price.
  Unsupported text fails closed as `UNKNOWN`. Existing historical rows are not
  rewritten, and an already running server remains on its loaded pre-change code
  until a separately controlled restart.
- **Project A sole trigger**：TradingView 仍只用 `POST /alert`。唯一 production
  producer 係 `LIQ_V2/r9 LIQ_TOUCH`（ICMARKETS:XAUUSD、5m、standard candles）。
  fresh/confirmed/non-duplicate touch 會寫 append-only Project A research request，
  要求 MCP structured reads + screenshots；唔會入 legacy parser/wake/fanout。
  同一 evidence retry 唔會 re-wake；真 re-touch（新 source time/touch count）冇固定
  cooldown。`EXP_V3/r5`、`RENKO_V3_SNIPER/r1`、`EXP_SCANNER/r6` 同 non-touch
  LIQ lifecycle 只係 strict compatibility telemetry，唔 materialize、唔開 alert、
  唔投票、唔 capture、唔建立/晉級 story。Invalid recognized payload bounded
  fail-closed。provider、writer、Telegram、Notion、MT5 同 order 繼續 disabled。
  冇 natural LIQ touch 就記錄 `NATURAL LIQ ALERT VERIFICATION PENDING`。
